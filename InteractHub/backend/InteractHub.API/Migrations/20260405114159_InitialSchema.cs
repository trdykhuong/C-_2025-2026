...
namespace InteractHub.API.Migrations
{
    ...
    public partial class InitialSchema : Migration
    {
        ...
        {
            // Note: removed AlterColumn for Posts.Id because changing IDENTITY on an existing
            // column requires dropping/recreating the column. For a clean database the Posts
            // table will be created below with the proper identity; altering identity on an
            // existing column is not supported by SQL Server via ALTER COLUMN.

            // Create AspNetUsers table (required by Identity and referenced by other tables)
            ...
            // Create Posts table before tables that reference it
            migrationBuilder.CreateTable(
                name: "Posts",
                columns: table => new
                {
            ...
            migrationBuilder.InsertData(
                ...
                values: new object[,]
                {
                    { "role-admin-id", "1", "Admin", "ADMIN" },
                    { "role-user-id", "2", "User", "USER" }
                });
            ...
            // Foreign key for Posts -> AspNetUsers already created when Posts table was defined above.
        }
